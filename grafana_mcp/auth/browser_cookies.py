"""Read grafana_session cookie directly from installed browser cookie stores.
...
reuse an existing browser Grafana session instead of triggering an Okta push.
"""
...
GRAFANA_HOST = "grafana.cloudtrust.rocks"
GRAFANA_COOKIE_NAME = "grafana_session"
# Grafana SAML sessions typically last 8–12 hours; used as TTL for session cookies
_DEFAULT_TTL_MS = 8 * 60 * 60 * 1000
...
def try_read_browser_session() -> "Optional[Session]":
    ...
    for browser_name, browser_fn in browsers:
        if browser_fn is None:
            continue
        try:
            cookies = browser_fn([GRAFANA_HOST])
            session = _find_grafana_session(cookies, browser_name)
            if session:
                return session
        except Exception as e:
            print(f"[auth] {browser_name}: cookie read error — {e}", file=sys.stderr)

    print("[auth] No valid grafana_session found in any browser cookie store", file=sys.stderr)
    return None


def _find_grafana_session(cookies: list, browser_name: str) -> "Optional[Session]":
    """Scan a list of rookiepy cookies for a non-expired grafana_session."""
    from grafana_mcp.auth.session import Session, save_session

    now_ms = int(time.time() * 1000)

    for c in cookies:
        if c.get("name") != GRAFANA_COOKIE_NAME:
            continue
        ...
        if expires_ms <= now_ms:
            print(
                f"[auth] {browser_name}: grafana_session cookie is expired — skipping",
                file=sys.stderr,
            )
            continue
        ...
        print(
            f"[auth] {browser_name}: grafana_session found, valid for {minutes_left} min",
            file=sys.stderr,
        )

        session = Session(grafana_session=value, expires_at=expires_ms)
